...
export default function PlaylistsPage() {
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      ...
      setCreateProgressText(
        "Finalizing and registering playlist in database...",
      );
  ...
  const handleAttachVideoToPlaylist = async (
    ...
    try {
      const videoKey = await uploadToImageKitWithProgress(
        file,
        "/playlists/videos",
      );
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({}));
        throw new Error(err.message || "Failed to attach video to playlist");
      }
    } catch (err: any) {
      const friendly = getFriendlyUploadErrorMessage(err);
      showToast(`${friendly.title}: ${friendly.message}`, "error");
  ...
  const handleRemoveVideoFromPlaylist = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to remove the video canvas from this playlist?",
      )
    )
      return;
    ...
    try {
      ...
      showToast(
        "Failed to remove video: " + (err.message || "Unknown error"),
        "error",
      );
    } finally {
  ...
  const handleDeletePlaylist = async (id: string, name: string) => {
    ...
    try {
      const res = await adminFetch(`/admin/playlist/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleAddSongToPlaylist = async (songId: string) => {
    ...
    try {
      const res = await adminFetch(
        `/admin/playlist/${selectedPlaylist.id}/songs`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleRemoveSongFromPlaylist = async (
    ...
    try {
      const res = await adminFetch(
        `/admin/playlist/${selectedPlaylist.id}/songs/${songId}`,
        {
          method: "DELETE",
        },
      );
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <p className="text-zinc-400 mt-1">
            Manage public playlists, background videos, and track composition.
          </p>
        </div>
        <button
            ...
            <div className="bg-[#121212] rounded-3xl border border-[#282828] p-8 space-y-6 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-[#282828] pb-6">
                <div className="flex items-center gap-6">
                  <div className="w-24 h-24 rounded-2xl bg-black/60 border border-[#282828] overflow-hidden shrink-0 shadow-lg">
                    {selectedPlaylist.coverImageKey && (
                      <img
                        src={getImageUrl(selectedPlaylist.coverImageKey, {
                          width: 200,
                          height: 200,
                          focus: "auto",
                          aspectRatio: "1-1",
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <label
                      className={`cursor-pointer bg-black/60 hover:bg-white hover:text-black border border-[#282828] text-zinc-300 px-4 py-2 rounded-full text-xs font-bold transition-all flex items-center gap-2 ${
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <option value="" className="bg-[#121212] text-zinc-400">
                  ...
                  {playlistSongs.length === 0 ? (
                    ...
                    playlistSongs.map((song) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleRemoveSongFromPlaylist(song.id, song.title)
                          }
                ...
                <div className="grid grid-cols-3 gap-2.5 w-full max-w-md">
                  ...
                  >
                    <div className="flex items-center gap-1 font-bold text-[11px] mb-0.5">
                      {createStep > 1 ? (
                        ...
                        <span className="w-3.5 h-3.5 rounded-full bg-white text-black flex items-center justify-center text-[9px]">
                          1
                        </span>
                      )}
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-75">
                      {newPlaylist.videoFile ? "Canvas video" : "Optional"}
                    </span>
                  </div>
              ...
              {(creating || createError) && (
                <div className="mt-4">
                  <UploadProgressBar
                    ...
                    statusText={
                      createProgressText || "Uploading playlist media..."
                    }
                    loadedText={uploadStats.loadedText}
                    speedText={uploadStats.speedText}
                    variant="inline"
                    error={createError}
                    onRetry={() => handleCreatePlaylist()}
                    onCancel={() => {
                      setCreateError(null);
                      setRetryStatusText("");
                    }}
