...
function SongsContent() {
  ...
  // Error & Retry states for Create & Edit flows
  ...
  const [editRetryStatusText, setEditRetryStatusText] = useState("");
  ...
  const handleCloseCreateModal = () => {
    if (uploading && !uploadError) {
      if (
        !confirm("An upload is in progress. Are you sure you want to cancel?")
      ) {
        return;
      }
    }
  ...
  const handleToggleFeatured = async (song: Song) => {
    ...
    try {
      ...
      if (res.ok) {
        setSongs((prev) =>
          prev.map((s) =>
            s.id === song.id ? { ...s, isFeatured: newFeatured } : s,
          ),
        );
        showToast(
          newFeatured ? "Track marked as featured" : "Track unfeatured",
          "success",
        );
      } else {
  ...
  const handleUpload = async (e?: React.FormEvent) => {
    ...
    if (!isDeviceOnline()) {
      ...
      setUploadError(offlineErr);
      setUploading(true);
      showToast(offlineErr.message, "error");
      return;
    }

    if (uploadMode === "videoOnly") {
      if (!formData.fullVideoFile)
        return showToast("Please select a full video file", "error");
    } else {
      if (!formData.songFile)
        return showToast("Please select an audio file", "error");
    }
    if (!formData.imageFile)
      return showToast("Please select a cover image", "error");
    if (!formData.title.trim())
      return showToast("Please enter a song title", "error");
    if (!formData.artistName.trim())
      return showToast("Please enter an artist name", "error");
    ...
    setUploadProgressText(
      uploadMode === "videoOnly"
        ? "Uploading full video to S3 storage..."
        : "Uploading audio file to S3 storage...",
    );
    ...
    try {
      ...
      if (formData.songFile) {
        ...
        const songUrlRes = await adminFetch(
          "/webhook/internal/song-upload-url",
        );
        if (!songUrlRes.ok)
      ...
      if (formData.fullVideoFile) {
        ...
        if (!videoUrlRes.ok)
          throw new Error("Failed to get full video upload authorization");
      ...
      if (uploadMode === "audio" && formData.videoFile) {
        setUploadStep(2);
        setUploadProgressText(
          "Uploading canvas background loop to ImageKit CDN...",
        );
        setUploadPercent(0);
      ...
      setUploadProgressText(
        "Registering song & scheduling background processing...",
      );
      setUploadStats({});
      ...
      const finalizeRes = await adminFetch("/admin/song", {
        ...
        body: JSON.stringify({
          ...
          clipStartMin:
            uploadMode === "videoOnly"
              ? Number(formData.clipStartMin)
              : undefined,
          clipStartSec:
            uploadMode === "videoOnly"
              ? Number(formData.clipStartSec)
              : undefined,
          clipEndMin:
            uploadMode === "videoOnly"
              ? Number(formData.clipEndMin)
              : undefined,
          clipEndSec:
            uploadMode === "videoOnly"
              ? Number(formData.clipEndSec)
              : undefined,
          previewStartTime:
      ...
      if (finalizeRes.ok) {
        ...
        fetchSongs();
        showToast(
          "Track uploaded successfully! Background transcoding worker queued.",
          "success",
        );
      } else {
  ...
  const handleEditSubmit = async (e?: React.FormEvent) => {
    ...
    const editRetryHandler = (
      attempt: number,
      maxRetries: number,
      delayMs: number,
    ) => {
      setIsEditRetrying(true);
      setEditRetryStatusText(
        `Network issue detected. Retrying in ${Math.ceil(delayMs / 1000)}s (attempt ${attempt}/${maxRetries})...`,
      );
    };
    ...
    try {
      ...
      if (editFormData.removeFullVideo) {
        ...
        const reprocessRes = await adminFetch(
          `/admin/song/${editingSong.id}/reprocess-video`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      let previewStartTime: number | null = null;
      if (
        editFormData.previewStartMin !== "" ||
        editFormData.previewStartSec !== ""
      ) {
      ...
      let previewEndTime: number | null = null;
      if (
        editFormData.previewEndMin !== "" ||
        editFormData.previewEndSec !== ""
      ) {
  ...
  const inputCls =
    "w-full bg-black/60 border border-[#282828] rounded-xl px-4 py-2.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-white focus:ring-1 focus:ring-white/20 transition-all";
  const labelCls =
    "block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wider";
  const fileCls =
    "w-full text-xs text-zinc-400 file:mr-3 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-white file:text-black hover:file:bg-zinc-200 cursor-pointer";
            ...
            >
              ...
              <div className="relative aspect-square overflow-hidden bg-zinc-900">
                ...
                <div className="absolute top-2 left-2 flex flex-wrap gap-1.5 max-w-[90%] z-10">
                  ...
                  {song.previewStartTime != null && (
                    <span
                      title={`Preview: ${formatDuration(song.previewStartTime)} - ${song.previewEndTime != null ? formatDuration(song.previewEndTime) : "end"}`}
                      className="flex items-center gap-1 bg-zinc-800/90 border border-[#282828] text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm font-mono shrink-0"
                    >
                      <svg
                        className="w-2.5 h-2.5"
                        fill="currentColor"
                      ...
                      {formatDuration(song.previewStartTime)}-
                      {song.previewEndTime != null
                        ? formatDuration(song.previewEndTime)
                        : "end"}
                    </span>
                  )}
                </div>
          ...
          <div className="bg-[#121212] w-full max-w-5xl rounded-3xl shadow-2xl border border-[#282828] overflow-hidden flex flex-col text-white">
            <div className="px-6 py-4.5 border-b border-[#282828] flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  Upload a standard audio track or extract from a full video
                </p>
              </div>
              <button
                type="button"
                onClick={handleCloseCreateModal}
                className="text-zinc-400 hover:text-white hover:rotate-90 transition-all p-1 rounded-lg"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            <div className="px-6 py-2.5 bg-black/60 border-b border-[#282828] flex gap-2">
              ...
              >
                ...
                <span
                  className={`text-[10px] font-normal ${uploadMode === "audio" ? "text-zinc-600" : "text-zinc-500"}`}
                >
                  (Standard)
                </span>
              </button>
              ...
              >
                ...
                <span
                  className={`text-[10px] font-normal ${uploadMode === "videoOnly" ? "text-zinc-600" : "text-zinc-500"}`}
                >
                  (Auto-Audio & Canvas)
                </span>
              </button>
            ...
            <form
              ...
              className="p-6 md:p-7 space-y-5 relative"
            >
              ...
              {uploading && (
                <div className="absolute inset-0 z-30 bg-black/95 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-200">
                  <UploadProgressBar
                    ...
                    onCancel={handleDismissUploadError}
                    retryStatusText={retryStatusText}
                    isRetrying={isRetrying}
                  />
                  {!uploadError && (
                    <p className="text-xs text-zinc-500 mt-6">
                      Please keep this window open while files are being
                  ...
                  <div className="border border-[#282828] rounded-2xl p-4 bg-black/40">
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        Preview / Best Part Timing{" "}
                        <span className="text-zinc-500 normal-case font-normal text-[10px]">
                          (Optional)
                        </span>
                      </label>
                      <span className="text-[10px] text-zinc-300 font-bold bg-zinc-800 px-2 py-0.5 rounded-full border border-[#282828]">
                        Snack / Preview
                      </span>
                    </div>
                    <p className="text-[11px] text-zinc-400 mb-3">
                      Specify the timestamp snippet for audio previews (leave
                      blank for start):
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-[#121212] p-2.5 rounded-xl border border-[#282828]">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-zinc-400 mt-3">
                            :
                          </span>
                          <div className="flex-1">
                            <span className="text-[9px] text-zinc-400">
                  ...
                  {uploadMode === "videoOnly" ? (
                    <div className="space-y-3.5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="border-2 border-dashed border-[#282828] bg-black/50 rounded-2xl p-3.5">
                          <div className="flex items-center justify-between mb-1">
                            <label
                              className={
                                labelCls + " text-white mb-0 font-bold"
                              }
                            >
                              Full Video <span className="text-red-500">*</span>
                            </label>
                            <span className="text-[9px] text-white font-bold px-2 py-0.5 rounded-full bg-zinc-800 border border-[#282828]">
                              Shaka HLS
                            </span>
                          </div>
                          <p className="text-[10px] text-zinc-400 mb-2">
                            Music video file (.mp4, .mov, .mkv). Audio extracted
                            automatically.
                          </p>
                          <input
                            required
                        ...
                        <div className="border border-dashed border-[#282828] rounded-2xl p-3.5 bg-black/50 flex flex-col justify-between">
                          <div>
                            ...
                            <p className="text-[10px] text-zinc-400 mb-2">
                              Square album artwork (ImageKit optimized).
                            </p>
                          </div>
                          <input
                            required
                      ...
                      <div className="border border-[#282828] rounded-2xl p-4 bg-black/40">
                        ...
                        <p className="text-[11px] text-zinc-400 mb-3">
                          Timestamp range in full video to cut for looping
                          player canvas:
                        </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <div className="border border-dashed border-[#282828] rounded-2xl p-3.5 bg-black/50">
                        <label className={labelCls + " mb-1"}>
                          Audio Track <span className="text-red-400">*</span>
                        </label>
                        <p className="text-[10px] text-zinc-400 mb-2">
                          MP3, WAV, AAC, FLAC audio source.
                        </p>
                        <input
                          required
                          type="file"
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              songFile: e.target.files?.[0] || null,
                            })
                          }
                          className={fileCls}
                        />
                      ...
                      <div className="border border-dashed border-[#282828] rounded-2xl p-3.5 bg-black/50">
                        ...
                        <p className="text-[10px] text-zinc-400 mb-2">
                          Short looping video snippet for player background.
                        </p>
                        <input
                          type="file"
                      ...
                      <div className="border border-dashed border-[#282828] rounded-2xl p-3.5 bg-black/50">
                        ...
                        <p className="text-[10px] text-zinc-400 mb-2">
                          Complete video for multi-bitrate HLS/DASH streaming.
                        </p>
                        <input
                          type="file"
                          accept="video/*"
              ...
              <div className="pt-4 border-t border-[#282828] flex items-center justify-between">
                <div className="text-xs text-zinc-400 flex items-center gap-2">
                  ...
                  <span>
                    {uploadMode === "videoOnly"
                      ? "Mode: Video with automated audio & canvas extraction"
                      : "Mode: Standard multi-track audio upload"}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <button
                    ...
                    onClick={handleCloseCreateModal}
                    className="px-5 py-2.5 rounded-full border border-[#282828] text-zinc-300 font-bold text-xs hover:bg-white/10 transition-all disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
                  ...
                  >
                    {uploading ? (
                      <>
                        ...
                        <span>
                          {uploadProgressText || "Processing Upload..."}
                        </span>
                      </>
                    ) : uploadMode === "videoOnly" ? (
                      "Upload Video & Process Track"
                    ) : (
          ...
          <div className="bg-[#121212] w-full max-w-lg rounded-2xl shadow-2xl border border-[#282828] overflow-hidden text-white">
            <div className="px-6 py-5 border-b border-[#282828] flex items-center gap-4">
              {editingSong.imageKey && (
                <img
                  src={getImageUrl(editingSong.imageKey, {
                    width: 80,
                    height: 80,
                    focus: "auto",
                    aspectRatio: "1-1",
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-zinc-400">
                  {editingSong.artistName}
                </p>
              </div>
              <button
                onClick={handleCloseEdit}
                className="text-zinc-400 hover:text-white hover:rotate-90 transition-all shrink-0"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 grid grid-cols-2 gap-3 bg-black/40 p-3.5 rounded-xl border border-[#282828]">
                  <div>
                    <label className={labelCls + " text-white mb-1"}>
                      Best Part Start
                    </label>
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                  ...
                  <div className="col-span-2 text-[11px] text-zinc-400">
                    Hover preview plays this segment (specified in minutes &amp;
                    seconds). Leave blank to disable.
                  </div>
                </div>
              </div>
                    ...
                    {editingSong.videoKey && (
                      <button
                        ...
                        onClick={() =>
                          setEditFormData({
                            ...editFormData,
                            removeVideo: !editFormData.removeVideo,
                            videoFile: null,
                          })
                        }
                    ...
                    {editingSong.fullVideoKey && (
                      <button
                        ...
                        onClick={() =>
                          setEditFormData({
                            ...editFormData,
                            removeFullVideo: !editFormData.removeFullVideo,
                            fullVideoFile: null,
                          })
                        }
              ...
              <button
                disabled={uploading}
                type="submit"
                className={`w-full py-3 rounded-full font-bold text-black text-sm transition-all flex items-center justify-center gap-2 ${uploading ? "bg-zinc-400 cursor-not-allowed" : "bg-white hover:bg-zinc-200"}`}
              >
                {uploading ? (
                  <>
                    <svg
