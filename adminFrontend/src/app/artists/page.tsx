...
export default function ArtistsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure? This will delete the artist and potentially affect associated songs.",
      )
    )
      return;
    try {
      ...
      showToast(
        err?.message || "Delete failed due to a network error",
        "error",
      );
    }
  };
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full p-20 text-center text-zinc-500 font-medium">
            No artists found.
          </div>
        ) : (
          artists.map((artist) => (
            <div
              key={artist.id}
              className="bg-[#121212] rounded-3xl border border-[#282828] overflow-hidden shadow-sm hover:border-zinc-600 transition-all group p-6"
            >
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-xl font-bold text-white truncate group-hover:text-zinc-200 transition-colors">
                    {artist.name}
                  </h3>
                  <p className="text-xs text-zinc-400 mt-1 line-clamp-2">
                    {artist.about || "No biography available."}
                  </p>
                </div>
              </div>
